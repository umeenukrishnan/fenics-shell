...
Ms = np.linspace(0.0, 1.0*M_max, 20)
...
ax = plt.gca()
ax.plot(Ms_analytical, vs/length, "-", color='blue', label="Analytical solution")
ax.plot(Ms_analytical, ws/length, "-", color='blue',)
ax.plot(Ms/M_max, v_hs/length, "x", color='green', label="$v_h/L$")
ax.plot(Ms/M_max, w_hs/length, "o", color='red', label="$w_h/L$")
ax.set_xlabel("$M/M_{\mathrm{max}}$")
ax.set_ylabel("Normalised displacement")
ax.legend()
plt.tight_layout()
plt.savefig("output/cantilever-displacement-plot.pdf")
# plt.savefig("hypar_convergence-ndiv.png")


file = File("cantilever.pvd")
uvec = as_vector([v_h[0], v_h[1], w_h])
