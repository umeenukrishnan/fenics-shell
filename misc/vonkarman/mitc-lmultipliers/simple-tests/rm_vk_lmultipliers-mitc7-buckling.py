...
def b_K(theta, R_theta, r_t, U):
    r"""Return bilinear form related to constructing reduced rotation in element interiors.
    
    .. note:: For use in conjunction with DuranLiberman or MITC7 with space_type='lagrange'.
    
    .. math::
        b_K(\theta, R_h(\theta); \tilde{r}) := \sum_{K} \int_{K} (\theta -
        R_h(\theta)) \cdot \tilde{r} \; dx = 0 \quad \forall \tilde{r} \in Q_2
    """
    return inner((theta - R_theta), r_t)*dx

Pi_R = b_e(theta_, R_theta_, p_, U_L) + b_K(theta_, R_theta_, p_, U_L)
