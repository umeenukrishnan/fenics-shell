...
# meshfile = 'eta8-h001-5-50-250-250.xml'
# mesh = Mesh(meshfile)
...
# Frame angle
disp_max = 1.5 # max displacement in the middle of the edge (1.5 for 45 degree and Lx=4)
disp_incr = np.linspace(0.0, disp_max, 200) # displacement increment
...
for count,i in enumerate(disp_incr):

    print '------------------------------------------------'
    print 'Now solving load increment ' + repr(count) + ' of ' + repr(len(disp_incr))
    w_exp.c = i
    # Solving
    solver.solve()
    v_h, theta_h, w_h, R_theta_h, p_h = z_.split(deepcopy=True)
    vx, vy = v_h.split(deepcopy=True)
    ...
    current_angle = np.arctan(i/(0.5*L_x + vx(0.5*L_x, 0.5*L_y)))
    # current_angle_small = i/(0.5*L_x + vx(0.5*L_x, 0.5*L_y))
    print 'The current base angle in degree is: %s' % (current_angle*180.0/np.pi)
    ...
    energy = 0.246*lmbd**(-1./3.)*(2.0*current_angle)**(7./3.)
...
np.save('sag' + '-mesh_structured-div10', ls_sag)
np.save('angles' + '-mesh_structured-div10', ls_angle)
np.save('energy' + '-mesh_structured-div10', ls_energy)
np.save('meshdata' + '-mesh_structured-div10', [z_.vector().array().size, mesh.hmax(), mesh.hmin()])
